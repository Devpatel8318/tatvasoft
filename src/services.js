...
//     timeout: 12400000, responseType: "json",
// });
...
//     if (config.headers) { config.headers["Content-Type"] = "application/json"; config.headers["lang"] = 'en' }
//     if (config.headers["isDisableLoader"] !== true) { requests.push(config.url); showLoader(); }
//     return config;
// }, (error) => { alert(error); Promise.reject(error); });
...
//     }
//     else { return Promise.resolve(response.data.result); }
// },
//     (error) => {
...
// function showLoader() { document.body.classList.add("loader-open"); }
// function hideLoader() { document.body.classList.remove("loader-open"); }

// // remove completed request function removeRequest(req) 
// {
//     const i = requests.indexOf(req);
//     if (i >= 0) { requests.splice(i, 1); }
//     if (requests.length > 0) { showLoader(); }
//     else { hideLoader(); } if (req === conflictRequest) {
//         conflictRequest = "";
